...
sceneK = graph(width=400,height=150,title='Average KE',ymin=0, 
ymax=0.3,xmin=0,xmax=100,xtitle='time',ytitle='KE avg')
Kavegraph=gcurve(color= color.red)  # plot KE
scenePE = graph(width=400,height=150,
title= 'Pot Energy', 
ymin=-0.6,ymax=0.0,xmin=0,xmax=100,
...
def initialposvel():           # Initial positions, velocities
  ...
  for j in range(0,Natom):
   xc = 2*x[j] - 7   # Linear TF places spheres
...
def Forces(t, PE):                                   # Forces
    ...
    for i in range( 0, Natom-1 ):
       for j in range(i + 1, Natom):
          dx = x[i] - x[j]                         
          if (abs(dx) > L/2): dx = dx-sign(L, dx) # Image
          r2 = dx*dx                    
          if (r2 < r2cut):              
             if (r2==0.): r2 = 0.0001   # Avoid 0 denom
             invr2 = 1./r2   
...
def timevolution():
    ...
    initialposvel()                         
    PE = Forces(t1,PE)
    for i in range(0,Natom):  KE = KE + (vx[i]*vx[i])/2   
    t = 0
